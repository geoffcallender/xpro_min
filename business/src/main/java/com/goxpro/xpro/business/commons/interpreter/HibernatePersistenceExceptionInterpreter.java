...
import com.goxpro.xpro.business.commons.exception.DuplicatePrimaryOrAlternateKeyException;
import com.goxpro.xpro.business.commons.exception.LockAcquisitionException;
import com.goxpro.xpro.business.commons.exception.LockTimeoutException;
...
import com.goxpro.xpro.business.commons.logger.LoggerConstants;
...
public class HibernatePersistenceExceptionInterpreter implements IPersistenceExceptionInterpreter {
	...
	static private final Logger systemHealthLogger = LoggerFactory.getLogger(LoggerConstants.SYSTEM_HEALTH);
	...
	public BusinessException interpret(javax.persistence.PersistenceException pe) {
		...
		else if (pe instanceof javax.persistence.LockTimeoutException) {
			Throwable rootCause = ExceptionUtil.getRootCause(pe);
			systemHealthLogger.error("LockTimeoutException: " + rootCause);
			systemHealthLogger.error("stacktrace = " + ExceptionUtil.printStackTrace(pe));
			return new LockTimeoutException(rootCause);
		}
		else {
	...
	private BusinessException interpretEntityExistsException(javax.persistence.EntityExistsException eee) {
		...
		if (cause instanceof org.hibernate.exception.ConstraintViolationException) {
			BusinessException be = interpretConstraintViolationException(
	...
	private BusinessException interpretOtherPersistenceException(javax.persistence.PersistenceException pe) {
		...
		if (t instanceof org.hibernate.exception.ConstraintViolationException) {
			...
			Throwable rootCause = ExceptionUtil.getRootCause(pe);
			systemHealthLogger.error("LockAcquisitionException: " + rootCause);
			// TODO - If we find that there is a javax.persistence.xxxException counterpart to this that we could have
			// picked up higher up. We haven't yet because this exception is so hard to simulate in dev.
			systemHealthLogger.error("pe = " + pe.getClass().getCanonicalName());
			systemHealthLogger.error("stacktrace = " + ExceptionUtil.printStackTrace(t));
			return new LockAcquisitionException(rootCause);
		}
	...
	public BusinessException interpret(java.sql.SQLException sqle) {
		...
		// else if (dbType == DBType.DERBY)
			...
			if (sqlState.equals(SQL_STATE_H2_OR_DERBY_DUPLICATE_ENTRY)) {
